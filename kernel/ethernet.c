...
void arp_send_packet(uint8_t mac[6], uint8_t ip[4]){


    arp_packet_t *packet = (arp_packet_t*)malloc(sizeof(arp_packet_t));
    
    uint8_t macc[6];
    get_mac_addr(macc);
    for(int i =0;i<6;i++){
        packet->src_mac[i] = macc[i];
    }
    kprintf("\nDEST: %x   %x   %x   %x   %x   %x   ",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);

    kprintf("\nSRC: %x   %x   %x   %x   %x   %x   ",packet->src_mac[0],packet->src_mac[1],packet->src_mac[2],packet->src_mac[3],packet->src_mac[4],packet->src_mac[5]);
    ...
    //dump_packet(packet);
    //kprintf("\n %d . %d .  %d .  %d .  %d . %d ",packet->opcode,packet->ip_len,packet->mac_len,packet->hardware_type,packet->protocol);
    ethernet_send(mac,(uint8_t*)packet,sizeof(arp_packet_t),ETHERNET_TYPE_ARP);
...
void arp_handle(arp_packet_t* packet,int len){
    ...
    if(ntohs(packet->opcode) == ARP_REQUEST){
        ...
        if(_memcmp(packet->dst_ip,&my_ip,4)){
            ...
            ethernet_send(dst_mac,(uint8_t*)packet,sizeof(arp_packet_t),ETHERNET_TYPE_ARP);
...
void arp_lookup(uint8_t mac[6],uint8_t ip[4]){
...
int arp_search(uint8_t *ret, uint8_t *ip ){
    ...
    for(int i =0;i<512;i++){
        ...
        for(int j =0;j<6;j++){
            if(arp_table[curr_table].ip_addr[j] == entry[j]){
                calc++;
            }
            else{
                calc--;
        ...
        if(calc>0 && calc>=6){
            _memcpy(ret,&arp_table[curr_table].mac_addr,6);
            return 1;
        }
    }
    return 0;
